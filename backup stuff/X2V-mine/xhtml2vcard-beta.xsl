...
@@ TABLE suport header/axis/id

-->



<xsl:param name="Prodid" select='"-//suda.co.uk//X2V 0.6.24 (BETA)//EN"' />
...
-->	
	<xsl:if test="not($Anchor) or @id = $Anchor">
		...
		<xsl:choose>
			...
			<xsl:when test="normalize-space(.//*[contains(concat(' ',normalize-space(@class),' '),' fn ')]) = normalize-space(.//*[contains(concat(' ',normalize-space(@class),' '),' org ')])">
				<xsl:text>&#x0A;N:;;;;;</xsl:text>		
			</xsl:when>
...
<xsl:choose>
	...
	<xsl:when test="@cite != ''">
		<xsl:choose>
			<xsl:when test="substring-before(@cite,':') = 'http'">
	...
	<xsl:when test="@action != ''">
		<xsl:choose>
			<xsl:when test="substring-before(@action,':') = 'http'">
...
<xsl:choose>
	<xsl:when test="@abbr != ''">
		<xsl:call-template name="escapeText">
			<xsl:with-param name="text-string"><xsl:value-of select="normalize-space(@abbr)" /></xsl:with-param>
		</xsl:call-template>
	</xsl:when>
	<xsl:when test="@longdesc != ''">
		<xsl:call-template name="escapeText">
...
<xsl:template name="textProp">
    ...
    <!-- @@ "the first descendant element with that class should take
		...
		<xsl:choose>
			...
			<xsl:when test='local-name(.) = "th" and @abbr'>
				<xsl:call-template name="escapeText">
					<xsl:with-param name="text-string" select="normalize-space(@abbr)" />
				</xsl:call-template>
			</xsl:when>			
	...
	<xsl:if test=".//*[contains(concat(' ', @class, ' '), concat(' ', 'type', ' '))]">
		<xsl:choose>
			...
			<xsl:when test="local-name(.//*[contains(concat(' ', @class, ' '), concat(' ', 'type', ' '))]) = 'abbr'">
				<xsl:if test=".//*[contains(concat(' ', @class, ' '), concat(' ', 'type', ' ')) and contains(concat(' ', @title, ' '), concat(' ', $value, ' '))]">
					<xsl:value-of select="normalize-space($value)"/>
				</xsl:if>
			</xsl:when>
		</xsl:choose>
